...
def main():
    print("Initializing Spark Session for pre-compaction inspection...")
    spark = SparkSession.builder \
        .appName("IcebergPreCompactionInspection") \
    ...
    print(f"Inspecting pre-compaction table: {table_name}")
    ...
    json_path = os.path.join(results_dir, "pre_compaction_metrics.json")
    with open(json_path, 'w') as f:
        json.dump(pre_metrics_json, f, indent=2)
    print(f"Wrote pre-compaction metrics to {json_path}")

    # CSV: File Metrics
    files_csv_path = os.path.join(results_dir, "pre_compaction_file_metrics.csv")
    ...
    print(f"Wrote pre-compaction file metrics to {files_csv_path}")

    # CSV: Snapshot History
    snapshots_csv_path = os.path.join(results_dir, "pre_compaction_snapshot_history.csv")
    ...
    print(f"Wrote pre-compaction snapshot history to {snapshots_csv_path}")

    # CSV: File Size Distribution
    dist_csv_path = os.path.join(results_dir, "pre_compaction_file_size_distribution.csv")
    ...
    print(f"Wrote pre-compaction file size distribution to {dist_csv_path}")
